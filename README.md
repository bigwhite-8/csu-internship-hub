...
The repository is intended for academic and portfolio presentation on GitHub.
...
- **Community board** backed by Supabase data storage (post creation, likes, comments, and admin delete flow).
- **External resource listing** with categorized internship/employment links, including automatic fetch and fallback curated sources.
...
- **UI:** Tailwind CSS (CDN)
- **Data storage service:** Supabase (community post data read/write)
- **External data fetching:** Browser-side fetch + DOM parsing for external links
